      ...
      <b-col xl="5">
        <b-card no-body class="auth-card">
          <b-card-body class="px-3 py-5">
            ...
            <h2 class="fw-bold text-center fs-18">Welcome back</h2>
            <p class="text-muted text-center mt-1 mb-4">Sign in to your YourConferenceNetwork account.</p>
...
const handleSignIn = async () => {
  ...
  if (result) {
    try {
      const res = await HttpClient.post('/api/v1/auth/login/', credentials);
      const { access, refresh, user: userData } = res.data;
      useAuth.saveSession({ access, refresh, user: userData });
      redirectUser();
    } catch (e: any) {
      const detail = e.response?.data?.detail ?? e.response?.data?.error ?? 'Invalid credentials.';
      if (error.value.length === 0) error.value = detail;
    }
  }
};
