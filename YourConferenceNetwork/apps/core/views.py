...
def data_quality(request):
    ...
    return render(request, "core/data_quality.html", context)
...
def network_maps(request):
    ...
    return render(request, "core/network_maps.html")
...
def map_three_layer(request):
    ...
    top_org_labels = sorted(org_nodes, key=lambda o: len(org_ec_window.get(o, [])), reverse=True)[:25]
    nx.draw_networkx_labels(G, pos, labels={n: n[:18] for n in top_org_labels},
